...
module.exports = function(app){
    ...
    async function getMatchChartData(matchID){
        const Query = {
            text: "SELECT Map.mapnumber, Map.mapname, CASE WHEN Map.winnerstart='ct' THEN CASE WHEN R.winner = T.teamid THEN R.winnerscore ELSE R.loserscore END ELSE CASE WHEN R.winner = T.teamid THEN R.loserscore ELSE R.winnerscore END END as ctstartscore, CASE WHEN Map.winnerstart='t' THEN CASE WHEN R.winner = T.teamid THEN R.winnerscore ELSE R.loserscore END ELSE CASE WHEN R.winner = T.teamid THEN R.loserscore ELSE R.winnerscore END END as tstartscore, r.round, mp.tick, r.round+COALESCE(CASE WHEN mp.tick=0 then 1 else mp.tick end/nullif(MAX(mp.tick)OVER(PARTITION BY Map.mapnumber,r.round),0)::float,0) as roundProg, CASE WHEN Map.winnerstart='ct' THEN mp.probct else 1-mp.probct END as probabilitymap from rounds R inner join maps Map on Map.mapid = R.mapid inner join map_prob mp on mp.mapid = Map.mapid and mp.round = r.round inner join matches Mat on Mat.matchid = Map.matchid inner join teams T on T.name = Mat.winner where Mat.matchid = $1 order by Map.mapnumber, r.round, mp.tick ASC;",
            values: [matchID],
        } 
        const res = await pool.query(Query);
        return res.rows;

...
module.exports = function(app){
    ...
    async function getPlayerSummaryData(){
        const Query = {
            text: "SELECT T.name as teamname, Atck.name, count(Map.mapid) as maps, sum(Atck.xKill+Vict.xKill)/count(Map.mapid) as xKills, sum(((Atck.WPA-Vict.WPA)/Map.rounds*100)/2)/count(Map.mapid) rWPA, sum(((Atck.xWPA-Vict.xWPA)/Map.rounds*100)/2)/count(Map.mapid) as eXrWPA from (SELECT P.name, P.playerid, rs.mapid, sum(RS.probabilitychange) as WPA, sum(K.expectedkill) as xKill, sum(K.expectedkill*RS.probabilitychange) as xWPA from roundstates RS inner join players P on P.playerid = RS.attacker inner join kills K on K.mapid = RS.mapid and K.round = RS.round and K.tick = RS.tick inner join maps Ma on Ma.mapid = RS.mapid inner join matches Mat on Mat.matchid = Ma.matchid where RS.attacker != -1 and RS.victim != -1 and Mat.date > CURRENT_DATE - 31 group by P.name, P.playerid, rs.mapid) as Atck inner join (SELECT P.name, P.playerid, rs.mapid, sum(RS.probabilitychange) as WPA, sum(1-K.expectedkill) as xKill, sum((1-K.expectedkill)*RS.probabilitychange) as xWPA from roundstates RS inner join players P on P.playerid = RS.victim inner join kills K on K.mapid = RS.mapid and K.round = RS.round and K.tick = RS.tick inner join maps Ma on Ma.mapid = RS.mapid inner join matches Mat on Mat.matchid = Ma.matchid where RS.attacker != -1 and RS.victim != -1 and Mat.date > CURRENT_DATE - 31 group by P.name, P.playerid, rs.mapid) as Vict on Vict.mapid = Atck.mapid and Vict.name = Atck.name inner join (SELECT winnerrounds+loserrounds as Rounds, mapid, matchid from maps) as Map on Vict.mapid = Map.mapid inner join player_maps PM on PM.mapid = Map.mapid inner join teams T on T.teamid = PM.teamid where PM.playerid = Atck.playerid group by T.name, Atck.name having count(Map.mapid) > 2 order by eXrWPA DESC"
        } 
        const res = await pool.query(Query);
        return res.rows;
    }

...
module.exports = function(app){
    ...
    async function getMatchDayChartData(matchIDs){
        ...
        for(var i=0; i < matchIDs.length; i++){
            ...
            const Query = {
                text: "SELECT Map.mapnumber, r.round, CASE WHEN Map.winnerstart='ct' and Map.winnerid = T.teamid THEN  R.ctprobabilitymap ELSE R.tprobabilitymap END as probabilitymap from rounds R inner join maps Map on Map.mapid = R.mapid inner join matches Mat on Mat.matchid = Map.matchid inner join teams T on T.name = Mat.winner where Mat.matchid = $1 order by Map.mapnumber ASC;",

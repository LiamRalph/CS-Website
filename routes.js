...
module.exports = function(app){
    ...
    async function getMatchDayChartData(matchIDs){
        ...
        for(var i=0; i < matchIDs.length; i++){
            ...
            const Query = {
                text: "SELECT Map.mapnumber, rs.round, rs.tick, CASE WHEN Map.winnerstart='ct' THEN CASE WHEN RS.ct=Map.winnerid THEN RS.ctprobability ELSE RS.tprobability END WHEN Map.winnerstart='t' THEN CASE WHEN RS.t=Map.winnerid THEN RS.tprobability ELSE RS.ctprobability END END as probabilitymap,ROW_NUMBER() OVER(PARTITION BY mapnumber ORDER BY mapnumber,round, tick)::int as gameTick from roundstates RS inner join maps Map on Map.mapid = RS.mapid inner join matches Mat on Mat.matchid = Map.matchid where Mat.matchid = $1 order by Map.mapnumber ASC;",

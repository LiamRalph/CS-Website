...
module.exports = function(app){
    ...
    async function getPlayerSummaryData(){
        const Query = {
            text: "SELECT T.name as teamname, stats.playername as name, count(distinct stats.mapid) as maps, sum(stats.xKill)/count(distinct stats.mapid) as xKills, sum(stats.WPA)/count(distinct concat(stats.round, stats.mapid))*100 as rWPA, sum(stats.xWPA)/count(distinct concat(stats.round, stats.mapid))*100 as eXrWPA from ((SELECT P.name as playername, P.playerid as id, RS.probabilitychange as WPA, K.expectedkill as xKill, K.expectedkill*RS.probabilitychange as xWPA, K.round, K.mapid as mapid from players P inner join roundstates RS on P.playerid = RS.attacker inner join kills K on K.mapid = RS.mapid and K.round = RS.round and K.tick = RS.tick inner join maps Ma on Ma.mapid = RS.mapid inner join matches Mat on Mat.matchid = Ma.matchid where Mat.date > CURRENT_DATE - 31 and (mat.team1rank < 21 and mat.team2rank < 21))union all (SELECT P.name as playername, P.playerid as id, (-1*RS.probabilitychange) as WPA, 1-K.expectedkill as xKill, (K.expectedkill)*(RS.probabilitychange)*-1 as xWPA, K.round, K.mapid as mapid from players P inner join roundstates RS on P.playerid = RS.victim inner join kills K on K.mapid = RS.mapid and K.round = RS.round and K.tick = RS.tick inner join maps Ma on Ma.mapid = RS.mapid inner join matches Mat on Mat.matchid = Ma.matchid where Mat.date > CURRENT_DATE - 31 and (mat.team1rank < 21 and mat.team2rank < 21))) stats inner join player_maps PM on PM.mapid = stats.mapid and PM.playerid = stats.id inner join teams T on T.teamid = PM.teamid where stats.playername != 'World' group by T.name, stats.playername  having count(distinct stats.mapid) > 4 order by eXrWPA DESC"
        } 
        const res = await pool.query(Query);
        return res.rows;
    }
